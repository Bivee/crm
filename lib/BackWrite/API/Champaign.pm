package BackWrite::API::Champaign;
...
# attributes
has 'model' => sub { BackWrite::Model->load('Champaign') };
has 'param';
...
sub create {
    ...
    # load model
    $self->model( BackWrite::Model->load('Champaign') )
      unless $self->model;
    ...
    if ( $self->model && $self->model->error ) {
        $self->message( {
            ...
            text  => 'database error saving champaign ' . $self->model->error,
        });
    }
    else {
        $self->message( {
