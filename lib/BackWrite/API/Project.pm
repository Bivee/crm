...
# attributes
has 'model' => sub { BackWrite::Model->load('Project') };
has 'param';
...
sub create {
    ...
    # load model
    $self->model( BackWrite::Model->load('Project') )
      unless $self->model;
    ...
    if ( $self->model && $self->model->error ) {
        $self->message( {
            ...
            text  => 'database error saving project ' . $self->model->error,
        });
    }
    else {
        $self->message( {
